...
public class MergeMetadataIds extends BasicFunction {
    protected static final Logger logger = LogManager.getLogger(MergeMetadataIds.class);
    public final static FunctionSignature signatures[] = {
        new FunctionSignature(
            new QName("merge-metadata-ids", Module.NAMESPACE_URI, Module.PREFIX),
    ...
    public MergeMetadataIds(XQueryContext context, FunctionSignature signature) {
    ...
    public Sequence eval(Sequence[] args, Sequence contextSequence) throws XPathException {
        ...
        if (cc != null) {
            if (cc.getURI().equals(url.toString())) {
        ...
        Metas from = md.getMetas(url);
        if (from != null) {
            from.metas().forEach(meta -> {
                Object v = meta.getValue();
                if (v != null) {
                    pairs.put(meta.getKey(), v.toString());
                }
            });
            md.delMetas(XmldbURI.createInternal(from.getURI()));
        }

        if (cc != null) {
            cc.metas().forEach(meta -> {
                Object v = meta.getValue();
                if (v != null) {
                    System.err.println(""+meta.getKey()+" = "+v);
                    pairs.put(meta.getKey(), v.toString());
                }
            });
            md.delMetas(XmldbURI.createInternal(cc.getURI()));
        }

        Metas cre = md._addMetas(url.toString(), uuid);
        pairs.forEach(cre::put);
