...
public class ImportNewResourcesHandler extends DefaultHandler {
    ...
    public ImportNewResourcesHandler(DBBroker broker, RestoreListener listener, String dbBaseUri, BackupDescriptor descriptor, ImportAnalyzer master) {
        this.broker = broker;
...
//        }
        ...
        } else if(namespaceURI.equals(Namespaces.EXIST_NS) && "subcollection".equals(localName)) {
            restoreSubCollectionEntry(atts);
        } else if(namespaceURI.equals(Namespaces.EXIST_NS) && "deleted".equals(localName)) {
            restoreDeletedEntry(atts);
        } else if(namespaceURI.equals(Namespaces.EXIST_NS) && "ace".equals(localName)) {
            addACEToDeferredPermissions(atts);
        } else {
            if (!deferredPermissions.isEmpty() && deferredPermissions.peek() != null) {
    ...
    public void endElement(String namespaceURI, String localName, String qName) throws SAXException {
        ...
        boolean process = !deferredPermissions.isEmpty() && deferredPermissions.peek() != null;
    ...
    private DeferredPermission restoreCollectionEntry(Attributes atts) throws SAXException {
        ...
        try {
            ...
            //skip if not new
            if (!master.createCols.contains(name)) {
                currentCollection = broker.getCollection(collUri);
                return null;
            }
            ...
            master.createCOL.incrementAndGet();
...
//            return;
        ...
        if(subDescriptor != null) {
            ...
            try { 
                ...
                final ImportNewResourcesHandler handler = new ImportNewResourcesHandler(broker, listener, dbBaseUri, subDescriptor, master);
    ...
    private DeferredPermission restoreResourceEntry(Attributes atts) throws SAXException {
        ...
        //skip if not new
        if (currentCollection == null || !master.createDocs.contains(currentCollection.getURI().append(name).toString())) {
            return null;
        }
        ...
        try {
            ...
            try {
                if ("XMLResource".equals(type)) {
                    master.createXML.incrementAndGet();
...
//                    resource.getUpdateLock().release(Lock.READ_LOCK);
    ...
    class ResourceDeferredPermission extends org.exist.xquery.restore.deferred.AbstractDeferredPermission<DocumentImpl> {
