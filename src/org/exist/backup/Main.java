...
public class Main {
    ...
    private static Properties loadProperties() {
        // read properties
        final Path propFile = ConfigurationHelper.lookup("backup.properties").toPath();
    ...
    public static void process(final ParsedArguments arguments) {
        ...
        if (backupCollection.isPresent()) {
            ...
            if (collection.isEmpty()) {
                if (guiMode) {
                    final CreateBackupDialog dialog = new CreateBackupDialog(properties.getProperty(URI_PROP, DEFAULT_URI), properties.getProperty(USER_PROP, DEFAULT_USER), properties.getProperty(PASSWORD_PROP, DEFAULT_PASSWORD), Paths.get(preferences.get("directory.backup", System.getProperty("user.dir"))).toFile());
            ...
            if (!collection.isEmpty()) {
                try {
                    final Backup backup = new Backup(
                        ...
                        Paths.get(properties.getProperty(BACKUP_DIR_PROP, DEFAULT_BACKUP_DIR)).toAbsolutePath().toString(),
    ...
    private static void restoreWithoutGui(final String username, final String password, final Optional<String> dbaPassword, final Path f,
        ...
        try {
            restore.restore(listener, username, password, dbaPassword.orElse(null), f.toFile(), uri);
