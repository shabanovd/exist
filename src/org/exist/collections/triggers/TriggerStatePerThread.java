...
/**
 * Finite State Machine, managing the state of a Running trigger; allows to
 * avoid infinite recursions by forbidding another trigger to run where there is
 * already one; feature trigger_update . I implemented that when a trigger is
 * running , another trigger in the same Thread cannot be fired . There is a
 * second condition that when a trigger is running triggered by some document d,
 * even the same trigger cannot run on a different document .
 * 
...
public class TriggerStatePerThread {
    ...
    public static boolean verifyUniqueTriggerPerThreadBeforePrepare(DocumentTrigger trigger, XmldbURI modifiedDocument) {
        if (getTriggerRunningState() == NO_TRIGGER_RUNNING) {
            setTriggerRunningState(TRIGGER_RUNNING_PREPARE, trigger, modifiedDocument);
            return true;
        } else {
            return false;
        }
    ...
    public static boolean verifyUniqueTriggerPerThreadBeforeFinish(DocumentTrigger trigger, XmldbURI modifiedDocument) {
        ...
        if (getTriggerRunningState() == TRIGGER_RUNNING_PREPARE) {
            setTriggerRunningState(TRIGGER_RUNNING_FINISH, trigger, modifiedDocument);
            return true;
        } else {
            return false;
        }
