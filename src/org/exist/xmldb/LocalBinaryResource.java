...
public class LocalBinaryResource extends AbstractEXistResource implements ExtendedResource, BinaryResource, EXistResource {
	...
	public Object getExtendedContent() throws XMLDBException {
		...
		try {
			...
			blob = (BinaryDocument)getDocument(broker, LockMode.READ_LOCK);
			...
			if(blob!=null)
				{collection.getCollection().releaseDocument(blob, LockMode.READ_LOCK);}
			if(broker!=null)
				{pool.release(broker);}
	...
	public Date getCreationTime() throws XMLDBException {
		...
		try {
			broker = pool.get(user);
			final BinaryDocument blob = (BinaryDocument)getDocument(broker, LockMode.NO_LOCK);
