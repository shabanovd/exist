 ...
 * 
 * Implements a mixture between LFU (Last Frequently Used) and LRU (Last
 * Recently Used) replacement policies. The class uses reference counts to track
 * references to cached objects. Each call to the add method increments the
 * reference count of the object.
 * 
 * If the cache is full, the object to be removed is determined by decrementing
 * the reference count for each object until an object with reference count = 0
 * is found.
 * 
...
public class GClockCache<T extends Cacheable> implements Cache<T> {
    ...
    public GClockCache(int size, double growthFactor, double growthThreshold, String type) {
        ...
        this.items = (T[]) new Cacheable[size];
        this.map = new Long2ObjectHashMap<T>(size * 2);
    ...
    protected T removeOne(T item) {
        ...
        do {
            ...
            if (bucket > -1) {
                old = items[bucket];
                if (old != null) {
                    // LOG.debug(fileName + " replacing " + old.getKey() +
                    // " for " + item.getKey());
    ...
    public void resize(int newSize) {
        if (newSize < size) {
            ...
            T[] newItems = (T[]) new Cacheable[newSize];
    ...
    protected void shrink(int newSize) {
        flush();
        items = (T[]) new Cacheable[newSize];
        map = new Long2ObjectHashMap<T>(newSize * 2);
        size = newSize;
        count = 0;
        used = 0;
        accounting.reset();
