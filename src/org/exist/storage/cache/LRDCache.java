...
/**
 * A cache implementation based on a Least Reference Density (LRD) replacement
 * policy.
 * 
 * The class maintains a global reference counter, containing the sum of all
 * references in the cache. Each object has a timestamp, which is equal to the
 * number of global references at the time, the object has been added to the
 * cache.
 * 
...
public class LRDCache<T extends Cacheable> extends GClockCache<T> {
     ...
     * 
     * @see
     * org.exist.storage.cache.LFUCache#add(org.exist.storage.cache.Cacheable,
     * int)
     */
     ...
     * 
     * @see
     * org.exist.storage.cache.LFUCache#removeOne(org.exist.storage.cache.Cacheable
     * )
     */
    protected T removeOne(T item) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (old == null) {
                ...
                rd = old.getReferenceCount() / (double) (totalReferences - old.getTimestamp());
    ...
    /**
     * Periodically adjust items with large reference counts to give younger
     * items a chance to survive.
     */
